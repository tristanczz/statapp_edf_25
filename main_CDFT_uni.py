...
for i in range(1, 13):
    ...
    plot_cdf_t(EUROCORDEX_mois_tas, cdf_t_correction(modele_hist_mois[i]['tas'], EUROCORDEX_mois_tas, modele_hist_mois[i]['tas']), cdf_t_correction(modele_hist_mois[i]['tas'], obs_mois[i]['tas'], modele_hist_mois[i]['tas']), cdf_t_correction(modele_hist_mois[i]['tas'], obs_mois[i]['tas'], modele_fut_mois[i]['tas']))



mod_hist_corr = cdf_t_correction(modele_hist_mois[1]['tas'], obs_mois[1]['tas'], modele_hist_mois[1]['tas'])
fut_corr      = cdf_t_correction(modele_hist_mois[1]['tas'], obs_mois[1]['tas'], modele_fut_mois[1]['tas'])
EUROCORDEX_mois = EUROCORDEX[EUROCORDEX['date'].dt.month == 1]
EUROCORDEX_mois_tas = EUROCORDEX_mois['tas'].values
